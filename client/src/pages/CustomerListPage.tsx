...
import { Plus, Edit, Trash2, MessageCircle, Users, Eye, Phone, Mail } from 'lucide-react';
...
const CustomerListPage = () => {
  ...
  const stats = {
    ...
    withPhone: customers ? customers.filter(c => c.phone).length : 0,
    withEmail: customers ? customers.filter(c => c.email).length : 0,
  };
  ...
  const columns = [
    {
      ...
      render: (customer: ICustomer) => (
        <div>
          ...
          <div className="text-sm text-gray-500 dark:text-gray-400">{customer.nic}</div>
        </div>
      )
    },
    {
  ...
  return (
    <Layout title="Customers">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 flex items-center justify-between">
          <div>
            <p className="text-sm text-gray-500 dark:text-gray-400">With Phone Number</p>
