  ...
  try {
    ...
    if (employee) {
      ...
      if (orderDoc) {
        ...
        notifyJobAssigned(employee, orderDoc as any);
      }
    }
...
// @access  Private
const getTrashedJobs = async (req: Request, res: Response) => {
  try {
...
// @access  Private
const restoreJob = async (req: Request, res: Response) => {
  try {
    const job = await Job.findById(req.params.id);
    if (job) {
      await job.restore();
...
// @access  Private
const forceDeleteJob = async (req: Request, res: Response) => {
  try {
    const job = await Job.findById(req.params.id);
