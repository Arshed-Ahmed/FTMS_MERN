...
const createOrder = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    if (req.user && status !== 'Draft') {
      await deductStockForOrder(materialsUsed, order._id as mongoose.Types.ObjectId, req.user._id, session);
    }
    ...
    if (measurementSnapshot && Object.keys(measurementSnapshot).length > 0) {
      ...
      if (customerDoc) {
        customerDoc.measurementHistory.push({
          orderId: createdOrder._id as mongoose.Types.ObjectId,
    ...
    if (status !== 'Draft') {
      ...
      if (customerDoc) {
        notifyOrderCreated(customerDoc, createdOrder as any);
      }
    }
...
// @access  Private
const updateOrder = async (req: AuthenticatedRequest, res: Response) => {
  const {
    customer,
    style,
    fitOnDate,
  ...
  try {
    ...
    if (order) {
      ...
      const isStockReserved = (s: string) => s !== 'Draft' && s !== 'Cancelled';
      ...
      if (materialsUsed || (wasReserved !== willBeReserved)) {
        ...
        if (wasReserved) {
          ...
          if (materialsUsed || !willBeReserved) {
            if (req.user) {
              await revertStockForOrder(materialsToRevert, order._id as mongoose.Types.ObjectId, req.user._id, session);
            }
          }
        }
        ...
        if (willBeReserved) {
          ...
          if (materialsUsed || !wasReserved) {
            if (req.user) {
              await deductStockForOrder(materialsToDeduct, order._id as mongoose.Types.ObjectId, req.user._id, session);
            }
          }
        }
      ...
      if (measurementSnapshot && Object.keys(measurementSnapshot).length > 0) {
        ...
        if (customerDoc) {
          const existingEntryIndex = customerDoc.measurementHistory.findIndex(
      ...
      if (status && status !== oldStatus) {
        ...
        if (customerDoc) {
          if (status === 'Ready') {
            notifyOrderReady(customerDoc, updatedOrder as any);
            ...
            if (oldStatus === 'Draft' && status === 'Pending') {
               notifyOrderCreated(customerDoc, updatedOrder as any);
            } else {
               notifyOrderStatusChange(customerDoc, updatedOrder as any, status);
            }
          }
        }
      }
...
// @access  Private
const getTrashedOrders = async (req: Request, res: Response) => {
  try {
...
// @access  Private
const forceDeleteOrder = async (req: AuthenticatedRequest, res: Response) => {
  const session = await mongoose.startSession();
  session.startTransaction();
...
// @access  Public
const getPublicOrderTrack = async (req: Request, res: Response) => {
  try {
    ...
    if (order) {
      res.json({
        ...
        styleName: (order.style as any)?.name,
        customerName: (order.customer as any)?.firstName,
        image: (order.style as any)?.image
      });
    } else {
      res.status(404).json({ message: 'Order not found' });
    }
