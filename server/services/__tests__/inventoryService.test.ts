...
describe('Inventory Service', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    session = {} as mongoose.ClientSession; // Mock session object
  });
  ...
  describe('deductStockForOrder', () => {
    it('should deduct stock successfully', async () => {
      ...
      const mockSessionFn = jest.fn<any>().mockResolvedValue(mockMaterial);
      (Material.findById as jest.Mock).mockReturnValue({
        session: mockSessionFn,
      });

      await deductStockForOrder(materialsUsed as any, orderId, userId, session);
    ...
    it('should throw error if material not found', async () => {
      ...
      const mockSessionFn = jest.fn<any>().mockResolvedValue(null);
      (Material.findById as jest.Mock).mockReturnValue({
        session: mockSessionFn,
      });

      await expect(
        deductStockForOrder(materialsUsed as any, orderId, userId, session)
      ).rejects.toThrow(`Material not found: ${materialId}`);
    });
    ...
    it('should throw error if insufficient stock', async () => {
      ...
      await expect(
        deductStockForOrder(materialsUsed as any, orderId, userId, session)
      ).rejects.toThrow('Insufficient stock for material: Fabric');
    });
  });
  ...
  describe('revertStockForOrder', () => {
    it('should revert stock successfully', async () => {
      ...
      await revertStockForOrder(materialsUsed as any, orderId, userId, session);
